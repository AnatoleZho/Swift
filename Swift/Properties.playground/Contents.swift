...
/*
 简单来说，一个存储属性就是存储在特定类和结构体实例里的的一个常量或变量。存储属性可以是变量存储属性（用关键字 var 定义），也可以是常量存储属性（用关键字 let 定义）。
 可以在定义存储属性的时候指定默认值。也可以在构造过程中设置或修改存储属性的值，甚至修改常量存储属性的值。
 
 下面的例子定义了一个名为 FixedLengthRange 的结构体，该结构体用于描述整数的范围。且这个范围值悲怆后不能被修改。
 */
...
var rangeOfThreeItems = FixedLenthRange.init(firstValue: 0, length: 3)
//该区间表示整数0， 1， 2
...
let rangeOfFourItems = FixedLenthRange.init(firstValue: 0, length: 4)
//该区间表示整数0， 1， 2， 3
...
/*         3.延迟存储属性          */
/*
 延迟存储属性是指当第一被调用的时候才会计算其初始值的属性。在属性声明前使用 lazy 来表示一个延迟存储属性。
 
   注意：
      必须将延迟存储属性声明为变量（使用 var 关键字）；以为属性的初始值可能在实例构造完成之后才会得到。而常量属性在构造过程完成之前必须要有初始值，因此无法声明成延迟属性
 
 延迟属性很有用，当属性的值依赖于在实例构造过程结束之后才会知道影响值的外部因素时，或者当获取属性的初始值需要复杂大量计算式，，可以只在需要的时候计算它。

 下面的例子使用了延迟存储属性来避免复杂类中不必要的初始化。例子中定义了 DataImproter 和 DataManager 两个类。
 */
...
// DataImportter 实例 importer 属性还没有被创建

/* 
 DataManager 类包含一个名为 data 的存储属性，初始值是一个空的字符串（String）数组。这里没有给出全部代码，只知道 DataManager 类的目的是管理和提供对这个字符串数组的访问即可。
 DataManager 的一个功能是从文件导入数据，该功能由 DataImporter 类提供， DataImporter 完成初始化需要消耗不少时间：因为它的实例在初始化时可能要打开文件，还要读取文件内容到内存。
 DataManger 管理数据时可能不从文件中导入数据。所以当 DataManger 的实例被创建时，没必要创建一个 DataImporter 的实例，更明智的做法是第一次用到 DataImporter 时候才去创建它。
...
/*
 注意：
   如果一个被标记为 lazy 的属性在没有初始化时就同时被多个线程访问，则无法保证该属性值会被初始化一次
 */
...
/*         5.计算属性          */
//出存储属性外，类、结构体和枚举可以定义计算属性。计算属性不直接存储值，而是提供了一个 getter 和 一个 setter ，来间接获取和设置其他属性或变量的值。
struct Point {
    var x = 0.0, y = 0.0
...
/*         6.简化 setter 声明          */
//如果计算属性的 setter 没有定义表示心智的参数名，则可以使用默认名称 newValue。 下面是是使用简化的 setter 声明的 Rect 结构体
...
/*
   注意
    必须使用 var 关键字定义计算属性，包括只读属性，因为它们的值不是固定的。let 关键字只用来声明常量属性，表示初始化后在无法修改的值
 */
...
let fourByFiveByTwo = Cubiod.init(width: 5.0, height: 5.0, depth: 2.0)
...
/*         8.属性观察器          */
/*
 属性观察器监控和相应属性值的变化，每次属性被设置值的时候都会调用属性观察器，即使使用新值和当前值相同的时候也不例外。
 
 可以为除了延迟存储属性之外的其他存储属性添加属性观察器，也可以听过重写属性的方法为集成的属性（包括存储属性和计算属性）添加属性观察器
。不必为非重写的计算属性添加属性观察器，因为可以通过它的 setter 直接监控和相应值的变化。
